...
smrExpt <- readRDS(system.file("extdata",
                               "mouseSE_dev_tooth_count_length_geneData.rds",
                               # "mouseSE_dev_tooth_count_length.rds", 
                               package = "broadSeq"))
```
...
gene_location_sample <- getBM(attributes=gene_attributes, filters = "ensembl_gene_id",
                              values = rownames(smrExpt),uniqueRows = TRUE,
...
smrExpt <- smrExpt[gene_location_sample$ensembl_gene_id,]

rownames(gene_location_sample) <- gene_location_sample$ensembl_gene_id
rowData(smrExpt) <- gene_location_sample[rownames(smrExpt),]
```
...
```{r warning=FALSE,eval=TRUE}
head(colData(smrExpt))
table(colData(smrExpt)$stage)
```
...
result_Noiseq <- 
    use_NOIseq(smrExpt = smrExpt, 
           class_id = "stage", control = "Bud", treatment = "Cap",
           rank = TRUE, 
...
pg <- broadSeq::genes_plot(smrExpt, x = "stage", y = "counts",  
           features = result_Noiseq %>% dplyr::filter(rank <5) %>% rownames(),
           fill="stage", facet.by = "mgi_symbol",
...
# limma 
?use_limma_trend(smrExpt, class_id, control, treatment, rank = FALSE, ...)
?use_limma_voom(smrExpt, class_id, control, treatment, rank = FALSE, ...)

# edgeR 
?use_edgeR_exact(smrExpt, class_id, control, treatment, rank = FALSE, ...)
?use_edgeR_GLM(smrExpt, class_id, control, treatment, rank = FALSE, ...)

# deseq2
?use_deseq2(smrExpt, class_id, control, treatment, rank = FALSE, ...)

# DELocal
?use_DELocal(smrExpt, class_id, control, treatment, rank = FALSE, ...)

# noiseq
?use_NOIseq(smrExpt, class_id, control, treatment, rank = FALSE, ...) 

# EBSeq 
?use_EBSeq(smrExpt, class_id, control, treatment, rank = FALSE, ...)

# samseq
?use_SAMseq(smrExpt, class_id, control, treatment, rank = FALSE, ...)
```
...
funs <- list(limma_trend = use_limma_trend, limma_voom = use_limma_voom,
             ...
             EBSeq = use_EBSeq, samseq = use_SAMseq) 


multi_result <- broadSeq::use_multDE(
    smrExpt = smrExpt[rowData(smrExpt)$chromosome_name == 5,], 
...
```{r warning=FALSE,eval=FALSE}
head(multi_result)
# nrow(multi_result) == nrow(smrExpt)
...
multi_result %>% broadSeq::volcanoPlot(
    ...
    labelName = "mgi_symbol",
    palette = c("purple","orange","grey"),
    selectedLabel =
        multi_result %>% dplyr::arrange(deseq2_padj) %>% pull(mgi_symbol) %>% head()
)
...
<!-- #### Boxplot -->
```{r warning=FALSE,fig.width=6,fig.height=6,eval=FALSE}
multi_result %>% dplyr::arrange(deseq2_padj) %>% pull(ensembl_gene_id) %>% head()
genes_plot(smrExpt, features = c("ENSMUSG00000035095","ENSMUSG00000037217"),
...
DELocal::plotNeighbourhood(pSmrExpt = smrExpt,pDesign = ~ condition, 
...
for (gene in head(top_delocal_high_dev_gene)) {
 DELocal::plotNeighbourhood(pSmrExpt = smrExpt,pDesign = ~ condition, pGene_id = gene)$plot %>% print()
}
...
gPlot <- DELocal:::plotNeighbourhood(smrExpt,pDesign = ~ condition, pGene_id = "ENSMUSG00000024598")
